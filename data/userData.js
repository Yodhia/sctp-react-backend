...
async function createUser({ name, email, password, salutation, country, marketingPreferences }) {
    // getting a connection so that we can begin a transaction
    const connection = await pool.getConnection();
    try {
        // begin a new transaction -- all changes to the database is temporary
        ...
        await connection.rollback(); // if any errors occur in the try block, undo all database changes since beginTransaction()
